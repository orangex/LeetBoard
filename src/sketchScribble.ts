...
import { modeScribbling } from "./index";
...
const sketchScribble = (pInst: p5) => {
    ...
    pInst.setup = function () {
        ...
        scribbledock.mouseClicked((ev: MouseEvent) => {
            ...
            let old = pInst.select(`#scribble-${currentScribbleType}`, 'div');
            ...
            let n: p5.Element = pInst.select(`#scribble-${scribbletype}`, 'div');;
    ...
    function createstrokeweightDiv() {
        let strokeweightArea = pInst.createDiv();
        strokeweightArea.class('subdock-toolscontainer')
        let light = pInst.createDiv();
        ...
        strokeweightArea.mouseClicked((ev: MouseEvent) => {
            ...
            let oldselected = pInst.select(`#${scribblePenArgsMap.get(currentScribbleType).get('strokeweight' as StrokeType)}`);
            let target = ev.target
            if (!(target instanceof Element)) return;
            switch (target.id) {
    ...
    function createPaletteDiv() {
        ...
        palette.mouseClicked((ev: MouseEvent) => {
            let oldSelected = pInst.select(`#${scribblePenArgsMap.get(currentScribbleType).get('strokecolor')}`);
            ...
            switch (ev.target.id) {
                ...
                case 'subdock-palette-gray':
                    let newSelected = pInst.select(`#${ev.target.id}`);
                    if (oldSelected)
...
export let pInst = new p5(sketchScribble, 'scribbleSketchHolder');
